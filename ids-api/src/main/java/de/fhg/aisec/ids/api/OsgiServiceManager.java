...
public class OsgiServiceManager {
  ...
  final class ServiceBinding<T> {
    ...
    public ServiceBinding(
        Consumer<T> bindFunction, Runnable unbindFunction, Class<T> serviceClass) {
  ...
  public <T> void bindService(
      Class<T> serviceClass, Consumer<T> bindFunction, Runnable unbindFunction) {
    ...
    bundleContext.addServiceListener(
        serviceEvent -> {
          ...
          if (eventType == ServiceEvent.REGISTERED || eventType == ServiceEvent.UNREGISTERING) {
            ...
            Stream<String> objectClasses =
                Arrays.stream((String[]) serviceReference.getProperty(OBJECTCLASS));
            // Map the classes to the Lists of ServiceBindings
            Stream<List<ServiceBinding>> bindingLists =
                objectClasses.map(bindings::get).filter(Objects::nonNull);
