 ...
 * 
 * The whole message is supposed to look like this:
 * 
 * <code>
 * --msgpart
...
public class MultiPartOutputProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		multipartEntityBuilder.addPart(MultiPartConstants.MULTIPART_HEADER, new StringBody(rdfHeader, ContentType.APPLICATION_JSON));

		// Get the Exchange body and turn it into the second part named "payload"
		InputStream payload = exchange.getIn().getBody(InputStream.class);
		if (payload != null) {
			multipartEntityBuilder.addPart(MultiPartConstants.MULTIPART_PAYLOAD, new InputStreamBody(payload,
