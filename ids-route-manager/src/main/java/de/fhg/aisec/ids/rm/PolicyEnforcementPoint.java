...
public class PolicyEnforcementPoint implements AsyncProcessor {
	...
	public boolean processFlowControl(Exchange exchange) {
		...
		PolicyDecision decision = pdp.requestDecision(new DecisionRequest(sourceNode, destNode,
				exchange.getProperties(), null));
		...
		switch (decision.getDecision()) {
			...
			default:
				if (LOG.isWarnEnabled()) {
					LOG.warn("Exchange blocked by data flow policy. Source: {}, Target: {}", sourceNode, destNode);
				}
	 ...
	 */
	private void applyLabelTransformation(TransformationDecision requestTransformations, Exchange msg) {
		// Remove labels from exchange
		requestTransformations.getLabelsToRemove().stream().map(l -> PDP.LABEL_PREFIX + l)
				.forEach(msg::removeProperty);

		// Add labels to exchange
		requestTransformations.getLabelsToAdd().stream().map(l -> PDP.LABEL_PREFIX + l)
