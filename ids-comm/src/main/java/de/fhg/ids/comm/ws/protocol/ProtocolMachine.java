...
public class ProtocolMachine {
	...
	protected Transition makeConsumerErrorTransition(ProtocolState state, ErrorHandler errorHandler) {
		return new Transition(ConnectorMessage.Type.ERROR,
				state, ProtocolState.IDSCP_END,
	...
	public FSM initIDSConsumerProtocol(WebSocket ws, IdsAttestationType attestationType, int attestationMask) {
		...
		// all handler
		RemoteAttestationConsumerHandler ratConsumerHandler = new RemoteAttestationConsumerHandler(
				fsm, attestationType, attestationMask, ttp, socket);
		...
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_START,
				ProtocolState.IDSCP_START, ProtocolState.IDSCP_RAT_AWAIT_REQUEST,
				e -> replyProto(ratConsumerHandler.enterRatRequest(e))));
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_REQUEST,
				ProtocolState.IDSCP_RAT_AWAIT_REQUEST, ProtocolState.IDSCP_RAT_AWAIT_RESPONSE,
				e -> replyProto(ratConsumerHandler.sendTPM2Ddata(e))));
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_RESPONSE,
				ProtocolState.IDSCP_RAT_AWAIT_RESPONSE, ProtocolState.IDSCP_RAT_AWAIT_RESULT,
				e -> replyProto(ratConsumerHandler.sendResult(e))));
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_RESULT,
				ProtocolState.IDSCP_RAT_AWAIT_RESULT, ProtocolState.IDSCP_RAT_AWAIT_LEAVE,
				e -> {
		...
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_LEAVE,
				ProtocolState.IDSCP_RAT_AWAIT_LEAVE, ProtocolState.IDSCP_META_REQUEST,
				e -> replyProto(metaHandler.request(e))));
		fsm.addTransition(new Transition(ConnectorMessage.Type.META_RESPONSE,
				ProtocolState.IDSCP_META_REQUEST, ProtocolState.IDSCP_END,
				e -> {
		...
		ProtocolState[] errorStartStates = new ProtocolState[]{
		...
		Arrays.stream(errorStartStates).forEach(state ->
				fsm.addTransition(makeConsumerErrorTransition(state, errorHandler)));

		// Add listener to log state transitions
		fsm.addSuccessChangeListener((f, e) ->
...
//        System.out.println(graph);
	...
	public FSM initIDSProviderProtocol(Session sess, IdsAttestationType type, int attestationMask, File tpmdSocket) {
		...
		// all handler
		RemoteAttestationProviderHandler ratProviderHandler = new RemoteAttestationProviderHandler(
				fsm, type, attestationMask, ttp, socket);
		ErrorHandler errorHandler = new ErrorHandler();
		...
		// Remote Attestation Protocol
		fsm.addTransition(new Transition(ConnectorMessage.Type.RAT_REQUEST,
				ProtocolState.IDSCP_START, ProtocolState.IDSCP_RAT_AWAIT_RESPONSE,
		...
		fsm.addTransition(new Transition(ConnectorMessage.Type.META_REQUEST,
				ProtocolState.IDSCP_META_REQUEST, ProtocolState.IDSCP_END,
				e -> {
					System.out.println("SETTING META: " + e.getMessage().getMetadataExchange().getRdfdescription());
					fsm.setMetaData(e.getMessage().getMetadataExchange().getRdfdescription());
		...
		Arrays.stream(errorStartStates).forEach(state ->
				fsm.addTransition(makeProviderErrorTransition(state, errorHandler)));
...
//        System.out.println(graph);
	...
	// TODO: This two constants are just dummies, fill them with the real protocol when TTP is implemented
	 ...
	 * <p>
	 * The URI is constructed from host and port settings in the settings service.
	 *
	...
	private URI getTrustedThirdPartyURL() {
		...
		try {
			return new URI(TTP_URI_PROTOCOL + "://" + config.getTtpHost() + ":" + config.getTtpPort()
