...
public class ProtocolMachine {
  ...
  public FSM initIDSConsumerProtocol(
    ...
    try {
      ...
      RemoteAttestationConsumerHandler ratConsumerHandler =
          new RemoteAttestationConsumerHandler(fsm, attestationType, attestationMask, ttp, socket);
      ...
      fsm.addTransition(
          new Transition(
              ...
              (e) -> {
                fsm.setMetaResult(e.getMessage().getMetadataExchange().getRdfdescription());
                return true;
              }));
      // fsm.addTransition(new Transition(ConnectorMessage.Type.META_RESPONSE,
      // ProtocolState.IDSCP_META_REQUEST, ProtocolState.IDSCP_END, (e) -> {return true;} ));
      ...
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_START,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_START, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_RAT_AWAIT_REQUEST,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_REQUEST, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_RAT_AWAIT_RESPONSE,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_RESPONSE, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_RAT_AWAIT_RESULT,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_RESULT, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_RAT_AWAIT_LEAVE,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_LEAVE, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_META_REQUEST,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_META_REQUEST, true)));
      fsm.addTransition(
          new Transition(
              ConnectorMessage.Type.ERROR,
              ProtocolState.IDSCP_META_RESPONSE,
              ProtocolState.IDSCP_END,
              (e) -> errorHandler.handleError(e, ProtocolState.IDSCP_META_RESPONSE, true)));

      /* Add listener to log state transitions*/
      fsm.addSuccessfulChangeListener(
  ...
  public FSM initIDSProviderProtocol(
    ...
    try {
      ...
      RemoteAttestationProviderHandler ratProviderHandler =
          new RemoteAttestationProviderHandler(fsm, type, attestationMask, ttp, socket);
      ErrorHandler errorHandler = new ErrorHandler();
      ...
      fsm.addTransition(
          new Transition(
              ...
              (e) -> {
                System.out.println(
                    "SETTING META TO " + e.getMessage().getMetadataExchange().getRdfdescription());
                fsm.setMetaResult(e.getMessage().getMetadataExchange().getRdfdescription());
                return replyProto(metaHandler.response(e));
              }));
      // fsm.addTransition(new Transition(ConnectorMessage.Type.META_RESPONSE,
      // ProtocolState.IDSCP_META_RESPONSE, ProtocolState.IDSCP_END, (e) -> {return true;} ));
      ...
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_START, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_REQUEST, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_RESPONSE, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_RESULT, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_RAT_AWAIT_LEAVE, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_META_REQUEST, false);
                return replyAbort();
              }));
      fsm.addTransition(
          new Transition(
              ...
              ProtocolState.IDSCP_END,
              (e) -> {
                errorHandler.handleError(e, ProtocolState.IDSCP_META_RESPONSE, false);
                return replyAbort();
              }));
   ...
   *
   * <p>The URI is constructed from host and port settings in the OSGi preferences service.
   *
   * @return
  ...
  private URI getTrustedThirdPartyURL() throws URISyntaxException {
    ...
    //			ttpHost = prefs.get().getUserPreferences(Constants.PREFERENCES_ID).get("ttp.host",
    // "127.0.0.1");
    //			ttpPort = prefs.get().getUserPreferences(Constants.PREFERENCES_ID).get("ttp.port",
    // "31337");
    //		}
